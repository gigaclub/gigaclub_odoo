...
try:  # noqa: C901
    ...
    class MainController(http.Controller):
            ...
            async def create_not_created_categories(self, guild):
                with api.Environment.manage():
                    with registry(self.env.cr.dbname).cursor() as new_cr:
                        ...
                        not_created_category_ids = new_env[
                            "gc.discord.category"
                        ].search([("discord_channel_uuid", "=", False)])
        ...
        def start_discord_bot(self):
            company_id = request.env.user.company_id or request.env[
                "res.company"
            ].browse(1)
            if (
                ...
                self.loop.create_task(
                    self.bot_async_start(company_id.gc_discord_bot_token)
                )
                bot_thread = threading.Thread(
                    target=self.bot_loop_start, args=(self.loop,)
                )
                bot_thread.start()
                ...
                raise Exception(
                    _(
                        "Bot is Started or Discord Bot Token or Discord Server ID not set!"
                    )
                )
        ...
        def stop_discord_bot(self):
            ...
            if (
                ...
                raise Exception(
                    _(
                        "Bot is Stopped or Discord Bot Token or Discord Server ID not set!"
                    )
                )
            return "<script>window.close()</script>"
    ...
    _logger.error("discord module not found!")
