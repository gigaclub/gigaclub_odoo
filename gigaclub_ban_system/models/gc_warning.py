...
class GCWarning(models.Model):
    ...
    @api.depends("warning_type.ban_time", "user_id.warning_ids")
    def _compute_ban_time(self):
        for rec in self:
            ban_time = (
                (rec.warning_type.ban_time ** len(rec.user_id.warning_ids)) * 60 * 60
            )
            expiration_datetime = datetime.now() + timedelta(seconds=ban_time)
            rec.ban_expiration_datetime = expiration_datetime

    @api.depends("warning_type.expiration_time", "user_id.warning_ids")
    def _compute_expiration_time(self):
        for rec in self:
            expiration_time = (
                (rec.warning_type.expiration_time ** len(rec.user_id.warning_ids))
                * 60
                * 60
            )
